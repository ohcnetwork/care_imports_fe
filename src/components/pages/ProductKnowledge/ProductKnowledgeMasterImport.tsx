...
interface ProductKnowledgeMasterImportProps {
  facilityId?: string;
  initialCsvText: string;
  onBack: () => void;
}

export default function ProductKnowledgeMasterImport({
  facilityId,
  initialCsvText,
  onBack,
}: ProductKnowledgeMasterImportProps) {
  ...
  const runImport = async () => {
    ...
    const validRows = processedRows.filter(
      (row) => row.errors.length === 0 && selectedRowIds.has(row.rowIndex),
    );
  ...
  if (currentStep === "review") {
    return (
      <div className="max-w-7xl mx-auto">
        <Card>
          <CardHeader>
            <CardTitle>Product Knowledge Import — Master Review</CardTitle>
            <CardDescription>
            ...
            <div>
              ...
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2 text-sm text-gray-600">
                ...
                <div className="flex items-center gap-2">
                  <input
                    id="product-knowledge-master-select-all"
                    type="checkbox"
                  ...
                  />
                  <label htmlFor="product-knowledge-master-select-all">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            · Skipped: {results?.skipped ?? 0} · Failed: {results?.failed ?? 0}
          </CardDescription>
        </CardHeader>
        <CardContent>
